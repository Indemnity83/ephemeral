...
public class GuiHandlerMainMenu {
    ...
    public static void initGui(GuiScreen gui, List<GuiButton> buttonList) {
        for (GuiButton button : buttonList) {
            if (button instanceof GuiButtonHosting) {
                return;
            }
        ...
        GuiButtonHosting button = new GuiButtonHosting(BUTTON_ID, gui.width / 2 + 2, gui.height / 4 + 72);
        buttonList.add(button);
    }

    public static void onActionPerformed(GuiButton button) {
        if (button.id == BUTTON_ID) {
            Minecraft.getMinecraft().displayGuiScreen(new GuiMenuHosting());
        }
    }
}
